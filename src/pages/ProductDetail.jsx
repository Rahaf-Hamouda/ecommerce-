  ...
  if (!product) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-32 text-center">
        ...
        <Link to="/products" className="text-rose-500 hover:text-rose-600 transition-colors">&larr; Back to Collection</Link>
      </div>
    );
  }
        ...
        <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.2 }}>
          ...
          <div className="border-t border-rose-100 pt-8">
            <div className="flex items-end gap-3 mb-8">
              ...
              {product.inStock && <span className="text-sm text-sage-600 font-medium bg-sage-50 px-3 py-1 rounded-full mb-1">&check; In Stock</span>}
            </div>

            <div className="flex items-center gap-4 mb-6">
              <span className="text-sm font-medium text-gray-600">Quantity</span>
              <div className="flex items-center gap-0 bg-rose-50 rounded-full overflow-hidden">
                <motion.button whileTap={{ scale: 0.85 }} onClick={() => setQuantity(q => Math.max(1, q - 1))} className="p-2.5 hover:bg-rose-100 transition-colors" disabled={quantity <= 1}>
                  <Minus className="w-4 h-4 text-gray-600" />
                </motion.button>
                <span className="w-10 text-center font-semibold text-sm text-gray-800">{quantity}</span>
                <motion.button whileTap={{ scale: 0.85 }} onClick={() => setQuantity(q => Math.min(10, q + 1))} className="p-2.5 hover:bg-rose-100 transition-colors">
