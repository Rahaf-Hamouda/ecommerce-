    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          <p className="text-gray-400 mb-10">Time to fill it with something beautiful</p>
          <Link to="/products">
            ...
            {items.map((item, index) => (
              <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -100 }}
                ...
                <div className="flex-1 min-w-0">
                  <Link to={`/product/${item.id}`} className="font-display font-semibold text-gray-800 hover:text-rose-500 transition-colors line-clamp-1">{item.name}</Link>
                  <p className="text-xs text-gray-400 mt-1">{item.category}</p>
                  <p className="text-lg font-bold text-gray-900 mt-2">${(item.price * item.quantity).toFixed(2)}</p>
                </div>
                <div className="flex flex-col items-end gap-3">
                  <motion.button whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.8 }} onClick={() => removeFromCart(item.id)}
        ...
        <div className="lg:col-span-1">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-gray-400">Subtotal ({items.length})</span><span className="font-medium text-gray-700">${cartTotal.toFixed(2)}</span></div>
              ...
              <div className="flex justify-between border-t border-rose-100 pt-4"><span className="font-bold text-gray-900">Total</span><span className="font-bold text-gray-900 text-xl">${cartTotal.toFixed(2)}</span></div>
            </div>
            <Link to="/checkout">
