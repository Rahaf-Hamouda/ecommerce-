...
export default function Checkout() {
  ...
  const handlePaymentSubmit = (e) => { e.preventDefault(); setOrderNumber(generateOrderNumber()); clearCart(); setStep(3); window.scrollTo({ top: 0, behavior: 'smooth' }); };
              ...
              <motion.form key="payment" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                ...
                {paymentMethod !== 'card' && (
                  <div className="bg-rose-50 rounded-2xl p-6 text-center mb-6">
                    <p className="text-sm text-gray-600">{paymentMethod === 'apple' ? "You'll be redirected to Apple Pay" : "You'll be redirected to your digital wallet"}</p>
