...
import {
  CreditCard, Check, ArrowLeft, Lock, Sparkles,
  Smartphone, Wallet, ShieldCheck, Package
} from 'lucide-react';
...
export default function Checkout() {
  ...
  const generateOrderNumber = () => {
    return 'ELG-' + Date.now().toString(36).toUpperCase() + Math.random().toString(36).substring(2, 6).toUpperCase();
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 pt-32 text-center">
        ...
        >
          ...
          <motion.div
            ...
            className="bg-white rounded-3xl border border-rose-50 p-6 shadow-soft mb-8 text-left"
          >
            ...
            <div className="border-t border-rose-50 pt-4 space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-700 capitalize">
                  {paymentMethod === 'card' ? '💳 Credit Card' : paymentMethod === 'apple' ? '📱 Apple Pay' : '👛 Digital Wallet'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Status</span>
                <span className="font-medium text-sage-600 flex items-center gap-1">
          ...
          <motion.div
            ...
            className="bg-white rounded-3xl border border-rose-50 p-6 shadow-soft mb-8"
          >
              ...
              {['Confirmed', 'Processing', 'Shipped', 'Delivered'].map((s, i) => (
                <div key={s} className="flex flex-col items-center flex-1">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                    i === 0
                      ? 'bg-gradient-to-br from-rose-500 to-blush-500 text-white shadow-glow'
                      : 'bg-gray-100 text-gray-300'
                  }`}>
                    {i === 0 ? <Check className="w-4 h-4" /> : i + 1}
                  </div>
                  <span className={`text-[10px] mt-2 ${i === 0 ? 'text-rose-500 font-medium' : 'text-gray-300'}`}>
                    {s}
                  </span>
                </div>
              ))}
          ...
          <div className="bg-cream-50 rounded-2xl p-4 mb-8">
            <p className="text-xs text-gray-400">
              🔒 This is a <span className="font-semibold text-rose-400">demo store</span> — no real payment was processed
            </p>
          </div>
      ...
      <div className="flex items-center gap-3 mb-10">
        {[
          ...
          <div key={s.num} className="flex items-center gap-3">
            ...
            {i === 0 && (
              <div className={`w-12 h-0.5 rounded-full mx-2 ${step > 1 ? 'bg-rose-400' : 'bg-gray-200'}`} />
            )}
          </div>
        ))}
      </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-gray-500 mb-2 ml-1">Phone</label>
                    <input type="tel" name="phone" value={form.phone} onChange={handleChange} required placeholder="+962 7XX XXX XXXX" className={inputClass} />
                  </div>
                  <div className="sm:col-span-2">
            ...
            {step === 2 && (
              ...
              >
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" name="expiry" value={form.expiry} onChange={handleChange} placeholder="MM / YY" required maxLength="7" className={inputClass} />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-500 mb-2 ml-1">CVV</label>
                        <div className="relative">
                          <input type="text" name="cvv" value={form.cvv} onChange={handleChange} placeholder="123" required maxLength="4" className={inputClass} />
                          <Lock className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-300" />
                        </div>
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-500 mb-2 ml-1">Cardholder Name</label>
                      <input type="text" placeholder="RAHAF HAMOUDA" className={inputClass} />
                    </div>
                  </div>
                )}

                {/* Apple Pay / Wallet message */}
                {paymentMethod !== 'card' && (
                  <div className="bg-rose-50 rounded-2xl p-6 text-center mb-6">
                    <p className="text-sm text-gray-600">
                      {paymentMethod === 'apple'
                        ? '📱 You\'ll be redirected to Apple Pay to complete payment'
                        : '👛 You\'ll be redirected to your digital wallet'}
                    </p>
                    <p className="text-xs text-gray-400 mt-2">(Demo — no real redirect)</p>
                  </div>
                )}
                ...
                >
                  <Lock className="w-4 h-4" /> Place Order 🛍️
                </motion.button>
          ...
          >
              ...
              {items.map(item => (
                <div key={item.id} className="flex gap-3">
                  <div className="relative flex-shrink-0">
                    ...
                    <span className="absolute -top-1.5 -right-1.5 bg-rose-500 text-white text-[9px] font-bold w-5 h-5 rounded-full flex items-center justify-center">
                      {item.quantity}
                    </span>
                  </div>
            ...
            <div className="border-t border-rose-100 pt-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-700">${(cartTotal * 0.08).toFixed(2)}</span>
              </div>
              <div className="flex justify-between border-t border-rose-100 pt-3">
                <span className="font-bold text-gray-900">Total</span>
                <span className="font-bold text-gray-900 text-xl">${(cartTotal * 1.08).toFixed(2)}</span>
              </div>
            </div>
          </motion.div>
        </div>
