          ...
          {items.map((item, index) => (
            ...
            >
              <div className="relative">
                ...
                <motion.button whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.8 }} onClick={() => removeFromWishlist(item.id)}
                  className="absolute top-4 right-4 p-2.5 glass rounded-full shadow-md"
                >
                  <Trash2 className="w-4 h-4 text-rose-500" />
                </motion.button>
                <span className="absolute top-4 left-4 glass text-rose-600 text-[11px] font-semibold px-3 py-1.5 rounded-full backdrop-blur-md">{item.category}</span>
              </div>
              <div className="p-5">
                <Link to={`/product/${item.id}`}>
                  <h3 className="font-display font-semibold text-gray-800 mb-1.5 line-clamp-1 group-hover:text-rose-600 transition-colors text-[15px]">{item.name}</h3>
                </Link>
